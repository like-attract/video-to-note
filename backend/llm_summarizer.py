...
{source}"""
    ...
    def _analysis_prompt(title: str, draft: str) -> str:
        return f"""请为这份视频笔记补写一段独立的点评与分析，只输出可直接追加到笔记后的 Markdown。

从内容组织、论述清晰度、材料支撑、表达方式和信息价值等方面做平衡点评，说明值得注意的亮点、
局限或可进一步思考之处。可以补充有助于理解的稳定背景知识，但必须标明“模型补充，未联网核验”，
不得把它写成视频原话或已核实事实。语气中性自然，服务于理解内容，避免复述已有笔记。

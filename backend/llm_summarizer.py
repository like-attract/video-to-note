...
# 小标题开头的 [MM:SS] / [起点-终点]（用户反馈标题带时间更好读，但目录行不需要重复一遍）
_HEADING_TIMESTAMP_RE = re.compile(r"^\[[^\]]{1,25}\]\s*")
    ...
    def _section_heading(text: str) -> str:
        for line in text.splitlines():
            ...
            if stripped.startswith("## "):
                # 目录行前已经印过这一段的起止时间，标题里再带一份就是重复
                return _HEADING_TIMESTAMP_RE.sub("", stripped[3:].strip())
        return ""
...
- 每个 ## 或 ### 小标题都写成「## [起点-终点] 标题」，区间取该话题在材料里的首尾时间点，方便回看时定位

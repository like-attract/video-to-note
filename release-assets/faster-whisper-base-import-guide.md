...
`paraformer-zh` 是另一套引擎（sherpa-onnx），导入目录和 whisper 系不一样：**在 1.4.1 及以上版本**，下拉框选中 `paraformer-zh` 再点「手动导入模型」，弹窗会按三个组件分别给出下载页和放置目录，照着放即可（标点那一份可以不放，程序会自动降级为无标点转写，不影响出稿）。

> 还在用 1.4.0 或更早版本的话，那个按钮只会给你一个用不上的 `manual\paraformer-zh` 文件夹（程序不读它）——请按下表手工铺文件，或者升级到 1.4.1。

程序实际读取的是 `<程序目录>\workspace\_model_cache\sherpa\` 下这三个子目录：

...
- 笔记使用 ArkData RDB，现为 v3 schema，并处理迁移中断重入；新生成笔记保存模型、服务商、
  风格与思考设置快照，不保存密钥或 Base URL。
...
- HarmonyOS 已移除桌面端不存在的“时间轴覆盖自动修订”额外 LLM 请求；详细风格仍保留桌面端已有的点评阶段。
- OpenAI-compatible LLM 请求使用 SSE 流式接收，并兼容中转站回退为普通 JSON；单次读取超时仍有 5 分钟上限，且不做传输层自动重试，避免重复计费。
- DeepSeek `auto` 明确映射为 `thinking=enabled + high`，auto/high 总输出预算固定为 12000；
  仅当用户显式选择 `max` 时才进一步放大，避免中转将 auto 升级为 max 或因过大预算长期挂起。
